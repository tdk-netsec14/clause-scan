...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { setServerError(err.error || 'Registration failed'); }
  ...
  return (
    <div className="min-h-screen flex bg-white">
      ...
      <div className="hidden lg:flex w-[45%] bg-navy-900 items-center justify-center p-12 relative overflow-hidden">
        {/* Abstract SVGs */}
        <svg viewBox="0 0 400 400" className="absolute bottom-0 left-0 w-96 h-96 text-gold-500 fill-current opacity-10 transform scale-x-[-1]">
          <path d="M400,200 C400,89.543 310.457,0 200,0 L200,40 C288.366,40 360,111.634 360,200 L400,200 Z" />
        </svg>
        <div className="absolute top-20 right-20 w-32 h-32 border border-gold-500 rounded-full opacity-10"></div>
        <div className="absolute bottom-1/4 right-1/4 w-3 h-3 bg-gold-500 rounded-full opacity-20"></div>
      ...
      <div className="flex-1 flex flex-col justify-center p-6 lg:p-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm mx-auto">
          <div className="text-center mb-10">
            <h2 className="font-serif text-[28px] font-bold text-navy-950 mb-2">Create your account</h2>
            <p className="text-gray-500 text-sm">Start analyzing your contracts in seconds.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input 
                ...
                className={`w-full px-4 py-3.5 border rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow ${errors.name ? 'border-red-300' : 'border-gray-200'}`}
